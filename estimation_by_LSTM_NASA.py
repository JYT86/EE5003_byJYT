...
class LSTM(nn.Module):
    ...
    def __init__(self, in_dim, hid_dim, out_dim):
        super().__init__()
        self.rnn = nn.LSTM(input_size=in_dim, hidden_size=hid_dim, num_layers=5, batch_first=True)#x:[batch_size, step_time, in_dim]
...
if __name__ == '__main__':
    ...
    ji = LSTM(in_dim, hid_dim, out_dim).cuda()
    loss_func = nn.MSELoss()
    optimizier = optim.Adam(ji.parameters(), lr)

    for epoch in range(10000):
        output, (hn, cn) = ji(train)
    ...
    #predict
    pred_out, (h, c) = ji(test.reshape(1, -1, in_dim))
    # print(pred_out)
